...
const NODE_CONSTRAINT_HINTS = {
  "back::right_hem": "Ширина спинки (петли)",
  "front::right_hem": "Ширина переда (петли)",
  "sleeve_left::sleeve_underarm_left": "Ширина рукава вверху (петли)",
  "sleeve_right::sleeve_underarm_right": "Ширина рукава вверху (петли)",
  "sleeve_left::sleeve_cuff_left": "Ширина манжеты (петли)",
  "sleeve_right::sleeve_cuff_right": "Ширина манжеты (петли)",
  "back::left_hem": "Высота спинки (ряды)",
  "front::left_hem": "Высота переда (ряды)",
  "back::neck_center": "Глубина горловины спинки",
  "front::neck_center": "Глубина горловины переда",
  "back::neck_left": "Ширина горловины",
};
...
function BlueprintSVG({
  ...
  const isKeyNode = (nodeName) => {
    // ❌ Промежуточные точки с числовым суффиксом — скрыть
    if (/_\d+$/.test(nodeName)) {
      return false;
    }

    // ✅ Ключевые узлы (обновлённые названия)
    const keyPatterns = [
      "_hem", // left_hem, right_hem
      "_underarm", // left_underarm, right_underarm
      "_cut", // left_cut, right_cut (подрез)
      "_neck_center", // neck_center
      "_cuff_", // sleeve_cuff_left/right
      "_top_", // sleeve_top_left/right
      "neck_left", // neck_left (без суффикса)
  ...
  const getPath = () => {
    ...
    if (partCode === "back") {
      ...
      const hemL = getNodePos("left_hem");
      const hemR = getNodePos("right_hem");
      const cutL = getNodePos("left_cut");
      const cutR = getNodePos("right_cut");
      const underarmL = getNodePos("left_underarm");
      const underarmR = getNodePos("right_underarm");
      const shoulderL = getNodePos("left_shoulder") || underarmL;
      const shoulderR = getNodePos("right_shoulder") || underarmR;
      const neckL = getNodePos("neck_left");
      const neckR = getNodePos("neck_right");
    ...
    if (partCode === "front") {
      ...
      const underarmL = getNodePos("left_underarm");
      const underarmR = getNodePos("right_underarm");
      const neckL = getSectionPoints("left", "neck");
      const neckR = getSectionPoints("right", "neck");
      const neckC = getNodePos("neck_center");
      ...
      if (hasDetailedNeck) {
        let path = `M ${hemL.x} ${hemL.y}
        L ${cutL?.x ?? hemL.x} ${cutL?.y ?? hemL.y}
        L ${underarmL?.x ?? hemL.x} ${underarmL?.y ?? hemL.y}`;

        if (necklineType === "V" && neckC) {
          path += ` L ${neckL[0].x} ${neckL[0].y}
          L ${neckC.x} ${neckC.y}
          L ${neckR[0].x} ${neckR[0].y}`;
        } else {
          neckL.forEach((p) => (path += ` L ${p.x} ${p.y}`));
          neckR
            .slice()
            .reverse()
            .forEach((p) => (path += ` L ${p.x} ${p.y}`));
        }
        path += ` L ${underarmR?.x ?? hemR.x} ${underarmR?.y ?? hemR.y}
        L ${cutR?.x ?? hemR.x} ${cutR?.y ?? hemR.y}
        L ${hemR.x} ${hemR.y} Z`;
...
const NODE_NAMES_RU = {
  // === Спинка / Перед (общие) ===
  left_hem: "Подол (лев.)",
  right_hem: "Подол (прав.)",
  left_cut: "Подрез (лев.)",
  right_cut: "Подрез (прав.)",
  left_underarm: "Подмышка (лев.)",
  right_underarm: "Подмышка (прав.)",
  left_shoulder: "Плечо (лев.)",
  right_shoulder: "Плечо (прав.)",
  neck_left: "Горловина (лев.)",
  neck_right: "Горловина (прав.)",
  neck_center: "Центр горловины",

  // === Рукава ===
  sleeve_cuff_left: "Манжета (лев.)",
  sleeve_cuff_right: "Манжета (прав.)",
  sleeve_underarm_left: "Подрез рукава (лев.)",
  sleeve_underarm_right: "Подрез рукава (прав.)",
  sleeve_top_left: "Окат (лев.)",
  sleeve_top_right: "Окат (прав.)",
  sleeve_cut_left: "Подрез оката (лев.)",
  sleeve_cut_right: "Подрез оката (прав.)",

  // === Set-in: дополнительные точки проймы/плеча ===
  left_armhole: "Пройма (лев.)",
  right_armhole: "Пройма (прав.)",
};
...
// ===== MAIN BLUEPRINTS TAB =====
export default function BlueprintsTab({ projectId, onBoundsUpdate }) {
  const { addToast, ToastContainer } = useToast();
