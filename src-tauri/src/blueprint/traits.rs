...
pub trait SleeveType: Send + Sync {
    ...
    fn neck_decreases_back(&self, m: &ProjectMeasurements) -> (Vec<DecreaseGroup>, i32) {
        (vec![], 0)
    }
    fn neck_decreases_front(&self, m: &ProjectMeasurements) -> (Vec<DecreaseGroup>, i32) {
        (vec![], 0)
    }
    ...
    fn proyma_info(&self) -> (i32, i32, i32, i32) {
        (0, 0, 0, 0)
    } // cap_height, proyma_h, hem_half, after_proyma
}
...
pub trait Calculation {
    ...
    fn as_raglan(&self) -> Option<&RaglanCalculation> {
        None
    }

    /// Downcast to SetInSleeveCalculation (returns None if Raglan)
    fn as_set_in(&self) -> Option<&SetInSleeveCalculation> {
        None
    }
}
