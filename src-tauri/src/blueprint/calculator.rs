...
impl BlueprintCalculator {
    pub fn new(sleeve_type: &str) -> Self {
        let sleeve: Box<dyn SleeveType> = match sleeve_type {
            "set_in" => Box::new(SetInSleeve::new()),
            _ => Box::new(RaglanSleeve), // default
        };
        Self { sleeve }
    }
    ...
    fn calculate_raglan(&self, m: &ProjectMeasurements) -> Result<BlueprintCalculation, String> {
        ...
        let (back_rows, back_counts) = gen_raglan_decreases(armhole_row + 2, garment_length - 2, dec_back, r);
        let (front_rows, front_counts) = gen_raglan_decreases(armhole_row + 2, garment_length - 2, dec_front, r);
        ...
        // Viewbox
        let viewbox_w = (hem_stitches * 2 + dims.top_stitches * 2 + 100) as i32;
        ...
        let cut = dec_shoulder as f64;

        nodes.extend(vec![
            bp("back_left_hem", bcx - half_w, hem_y, "back"),
            bp("back_right_hem", bcx + half_w, hem_y, "back"),
            bp("back_left_underarm", bcx - half_w, underarm_y, "back"),
            bp("back_right_underarm", bcx + half_w, underarm_y, "back"),
            bp("back_left_shoulder", bcx - half_w + cut, underarm_y, "back"),
            bp("back_right_shoulder", bcx + half_w - cut, underarm_y, "back"),
            bp("back_left_raglan", bcx - neck_back_st as f64 / 2.0, neck_y, "back"),
            bp("back_right_raglan", bcx + neck_back_st as f64 / 2.0, neck_y, "back"),
            bp("back_neck_center", bcx, neck_y + (m.glg * r * 0.25) as f64, "back"),
        ]);

        // Front
        let fcx = (viewbox_w / 4) as f64;
        let shoulder_row = *neck_rows.iter().max().unwrap_or(&garment_length);
        let neck_y_shoulder = hem_y - shoulder_row as f64;

        nodes.extend(vec![
            bp("front_left_hem", fcx - half_w, hem_y, "front"),
            bp("front_right_hem", fcx + half_w, hem_y, "front"),
            bp("front_left_underarm", fcx - half_w, underarm_y, "front"),
            bp("front_right_underarm", fcx + half_w, underarm_y, "front"),
            bp("front_left_shoulder", fcx - half_w + cut, underarm_y, "front"),
            bp("front_right_shoulder", fcx + half_w - cut, underarm_y, "front"),
            bp("front_neck_left", fcx - neck_front_st as f64 / 2.0, neck_y_shoulder, "front"),
            bp("front_neck_right", fcx + neck_front_st as f64 / 2.0, neck_y_shoulder, "front"),
            bp("front_neck_center", fcx, hem_y - garment_length as f64, "front"),
        ]);
        ...
        nodes.extend(self.sleeve.generate_left_nodes(m, &RaglanCalculation {
            back_width_stitches: hem_stitches, front_width_stitches: hem_stitches,
            neck_width_stitches: neck_back_st, sleeve_top_stitches: dims.top_stitches,
            sleeve_cuff_stitches: dims.cuff_stitches, total_rows: garment_length,
            raglan_start_row_front: armhole_row, raglan_start_row_back: armhole_row,
            raglan_end_row: garment_length - 2, sleeve_height_rows: dims.height_rows,
            sleeve_increase_rows: dims.increase_rows.clone(),
            total_decreases: dec_back, neck_decrease_counts: neck_counts.clone(),
            neck_decrease_rows: neck_rows.clone(), decrease_shoulder_cuts: dec_shoulder,
            viewbox_width: viewbox_w, viewbox_height: viewbox_h,
            sleeve_shoulder_cut_rows: dims.shoulder_cut_rows,
            sleeve_slope_start_x: dims.slope_start_x,
            sleeve_slope_end_x: dims.slope_end_x,
        ...
        nodes.extend(self.sleeve.generate_right_nodes(m, &RaglanCalculation {
            back_width_stitches: hem_stitches, front_width_stitches: hem_stitches,
            neck_width_stitches: neck_back_st, sleeve_top_stitches: dims.top_stitches,
            sleeve_cuff_stitches: dims.cuff_stitches, total_rows: garment_length,
            raglan_start_row_front: armhole_row, raglan_start_row_back: armhole_row,
            ...
            neck_depth_rows: neck_depth,
            nodes: vec![],
            sleeve_raglan_rows_back: vec![],
            sleeve_raglan_rows_front: vec![],
        }, &dims, sleeve_cx));
        ...
        Ok(BlueprintCalculation::Raglan(RaglanCalculation {
            ...
            sleeve_increase_rows: dims.increase_rows,
            total_decreases: dec_back,
            neck_decrease_counts: neck_counts,
            neck_decrease_rows: neck_rows,
            ...
            sleeve_cap_offset: dims.cap_offset,
            sleeve_width_stitches: ((m.or_val + m.ease) * p).round(),
            back_decrease_rows: back_rows,
            back_decrease_counts: back_counts,
            front_decrease_rows: front_rows,
            front_decrease_counts: front_counts,
            neck_depth_rows: neck_depth,
            nodes,
    ...
    fn calculate_set_in(&self, m: &ProjectMeasurements) -> Result<BlueprintCalculation, String> {
        ...
        let ease = m.ease;
        let shoulder_len = m.shoulder_length as i32;

        // Самая широкая часть рукава
        ...
        let armhole_height_rows = (proyma_height_cm as f64 * r).round() as i32;
        ...
        let start_shoulder_slope_row = garment_len_rows - (shoulder_slope_height * r).round() as i32;
        let shoulder_len_st = (shoulder_len as f64 * p).round() as i32;
        let shoulder_decrease_times = (shoulder_slope_height / 2.0).round() as i32;
        ...
        // Полная ширина низа
        let hem_width = (hem_width_half_cm as f64 * p * 2.0).round() as i32;
        ...
        let (neck_decrease_rows_back, rem_back) = calculate_neckline_decreases(
            (m.oh / 2.0 / 2.0 * p).round() as i32,
            (m.glg / 2.0 * r).round() as i32,
        );
        let (neck_decrease_rows_front, rem_front) = calculate_neckline_decreases(
            (m.oh / 2.0 / 2.0 * p).round() as i32,
        ...
        let waist_decreases = calculate_waist_decreases(
            m.hip_circumference,
        ...
        let waist_increases = calculate_waist_increases(
            m.waist_circumference,
            m.og,
            p,
            back_len_rows / 2,
        ...
        let waist_start_row = hip_len_rows;
        let waist_end_row = back_len_rows;
        let waist_point_row = (m.back_len / 2.0 * r).round() as i32;

        // Viewbox
        let viewbox_width = 1200;
        let viewbox_height = 900;

        let result = SetInSleeveCalculation {
            ...
            start_shoulder_slope_row,
            shoulder_decrease_stitches: (shoulder_len_st as f64 / shoulder_decrease_times.max(1) as f64).round() as i32,
            shoulder_decrease_times,
            sleeve_cuff_stitches: cuff_st,
            sleeve_widest_stitches: sleeve_widest_st,
            sleeve_cap_height_rows,
            sleeve_cap_decreases: sleeve_cap_decreases.clone(),
            sleeve_body_rows: sleeve_body_rows.max(10),
            total_garment_rows: garment_len_rows,
            viewbox_width,
            viewbox_height,
            nodes: Vec::new(),
            neck_decreases_rows_back: neck_decrease_rows_back,
            neck_decreases_rows_front: neck_decrease_rows_front,
            rem_back,
            rem_front,
            shoulder_decreases,
            waist_decreases,
        ...
        Ok(BlueprintCalculation::SetIn(result))
    }
}
...
async fn load_measurements(project_id: i64, pool: &SqlitePool) -> Result<ProjectMeasurements, String> {
    let rows = sqlx::query("SELECT measurement_code, value FROM project_blueprint_measurements WHERE project_id = ?")
        .bind(project_id)
        .fetch_all(pool)
        .await
...
fn gen_u_neckline_decreases(neck_w: i32, neck_depth: i32) -> (Vec<i32>, Vec<i32>) {
    let (groups, _rem) = calculate_neckline_decreases(neck_w, neck_depth);
    decrease_groups_to_rows(&groups)
}

fn gen_sleeve_raglan_rows(dims: &SleeveDimensions, is_back: bool) -> Vec<i32> {
    let mut rows = Vec::new();
    if dims.increase_rows.is_empty() { return rows; }
    let offset = if is_back { 0 } else { 2 };
    for (i, &row) in dims.increase_rows.iter().enumerate() {
        rows.push(row + offset + i as i32);
    }
    rows
}
...
pub fn calculate_proyma_decreases(proyma_width: i32) -> Vec<DecreaseGroup> {
    ...
    if part1 > 0 {
        steps.push(DecreaseGroup { stitches: part1, every_n_rows: 1, repeat_count: 1 });
    }
    if part2 > 0 {
        let full = part2 / 3;
        let rem = part2 % 3;
        if full > 0 { steps.push(DecreaseGroup { stitches: 3, every_n_rows: 2, repeat_count: full }); }
        if rem > 0 { steps.push(DecreaseGroup { stitches: rem, every_n_rows: 2, repeat_count: 1 }); }
    }
    if part3 > 0 {
        let full = part3 / 2;
        let rem = part3 % 2;
        if full > 0 { steps.push(DecreaseGroup { stitches: 2, every_n_rows: 2, repeat_count: full }); }
        if rem > 0 { if let Some(l) = steps.last_mut() { l.stitches += rem; } }
    }
    if part4 > 0 {
        let base = part4 / 5;
        let rem = part4 % 5;
        if base > 0 { steps.push(DecreaseGroup { stitches: base, every_n_rows: 4, repeat_count: 5 }); }
        if rem > 0 { if let Some(l) = steps.last_mut() { l.stitches += rem; } }
    }
    steps
}
...
pub fn calculate_sleeve_cap_decreases(widest: i32, cap_height: i32) -> Vec<DecreaseGroup> {
    ...
    if h2 > 0 {
        ...
        while i >= 3 { threes.push(3); i -= 3; }
        if i > 0 { threes.push(i); }
        if threes.len() > 1 && threes.last().unwrap() < &3 {
            let v = threes.pop().unwrap(); *threes.last_mut().unwrap() += v;
        }
        for st in threes { steps.push(DecreaseGroup { stitches: st, every_n_rows: 2, repeat_count: 1 }); }
    }
    if h1 > 0 {
        let mut i = h1;
        let mut twos = Vec::new();
        while i >= 2 { twos.push(2); i -= 2; }
        if i > 0 { twos.push(i); }
        if twos.len() > 1 && twos.last().unwrap() < &2 {
            let v = twos.pop().unwrap(); *twos.last_mut().unwrap() += v;
        }
        for st in twos { steps.push(DecreaseGroup { stitches: st, every_n_rows: 2, repeat_count: 1 }); }
    }
    if p2 > 0 {
        let used: i32 = steps.iter().map(|s| s.every_n_rows * s.repeat_count).sum();
        let remaining = cap_height - used;
        let interval = if p2 > 0 && remaining > 0 { (remaining as f64 / p2 as f64).round().max(2.0) as i32 } else { 2 };
        for _ in 0..p2 { steps.push(DecreaseGroup { stitches: 1, every_n_rows: interval, repeat_count: 1 }); }
    }
    if p3 > 0 {
        let mut rem = 0;
        let mut p3 = p3;
        while p3 % 3 != 0 { p3 -= 1; rem += 1; }
        let mut i = p3;
        while i >= 3 { steps.push(DecreaseGroup { stitches: 3, every_n_rows: 4, repeat_count: 1 }); i -= 3; }
        if rem > 0 { if let Some(l) = steps.last_mut() { l.stitches += rem; } }
    }
    steps
}
...
// === Скос плеча ===
pub fn calculate_shoulder_decreases(shoulder_slope_height: f64, shoulder_len_stitches: f64) -> Vec<DecreaseGroup> {
    let shoulder_decreases_times = (shoulder_slope_height / 2.0).round() as i32;
    let mut steps = Vec::new();
    if shoulder_decreases_times <= 0 { return steps; }

    let mut decreases_count_stitches = shoulder_len_stitches / shoulder_decreases_times as f64;
    if decreases_count_stitches >= 0.5 { decreases_count_stitches += 1.0; }
    else { decreases_count_stitches -= 1.0; }
    ...
    if (shoulder_len_stitches.round() as i32) % shoulder_decreases_times > 0 {
        if let Some(last) = divisions.last_mut() { *last += 1; }
    }
    for i in divisions {
        steps.push(DecreaseGroup { stitches: i, every_n_rows: 2, repeat_count: 1 });
    }
    steps
}

// === Горловина (U-образная) — формула технолога ===
pub fn calculate_neckline_decreases(half_neck_width_stitches: i32, neck_height_rows: i32) -> (Vec<DecreaseGroup>, i32) {
    let mut steps: Vec<DecreaseGroup> = Vec::new();

    // Делим на 4 части — остаток идёт в part4 (подрез)
    ...
    // 2-я группа: делим на двойки, каждые 2 ряда
    if part2 > 0 {
        let twos = part2 / 2;
        let rem = part2 % 2;
        if twos > 0 {
    ...
    // 3-я группа: делим на тройки, каждые 2 ряда
    if part3 > 0 {
    ...
    // 4-я группа: по 1 петле каждые 2 ряда, 4 раза
    if part4 > 0 {
        let fors = part4 / 4;
        if fors > 0 {
            steps.push(DecreaseGroup {
    ...
    (steps, (neck_height_rows - half_neck_width_stitches).max(0))
}
